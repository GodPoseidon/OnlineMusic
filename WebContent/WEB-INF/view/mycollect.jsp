...
<script type="text/javascript" src="script/mycollect.js"></script>
<title>我的收藏</title>
</head>
<body>
...
<div id="all">
		...
		<c:forEach  var="item" items="${requestScope.mycollect}" varStatus="st">
		<tr>
		<td>${st.index+1}</td>
		<td>${item.song}</td>
		<td>${item.special}</td>
		...
		<td><a href="person/play?songid=${item.id}" class="play">播放</a><a href="download?songid=${item.id}">下载</a><a href="person/collectdelete?collectid=${item.collectId}" class="delete">删除</a></td>
		</tr>
		</c:forEach>
		</table>
		</div>
		...
		<div id="footer">
		<form action="person/mycollect" method="post">
